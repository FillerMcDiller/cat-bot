...
ALTER TABLE public.item OWNER TO cat_bot;
...
CREATE INDEX IF NOT EXISTS idx_item_guild_user_key ON public.item USING btree (guild_id, user_id, item_key);
...
-- Tables added after the original dump. These ALTER statements make the
-- schema repair existing installations as well as creating fresh tables.
...
ALTER TABLE public."user"
    ADD COLUMN IF NOT EXISTS dm_ach_sent integer DEFAULT 0;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'cat_bot') THEN
        GRANT USAGE ON SCHEMA public TO cat_bot;
        GRANT SELECT, INSERT, UPDATE, DELETE, REFERENCES, TRIGGER
            ON ALL TABLES IN SCHEMA public TO cat_bot;
        GRANT USAGE, SELECT, UPDATE
            ON ALL SEQUENCES IN SCHEMA public TO cat_bot;
    END IF;
END $$;
